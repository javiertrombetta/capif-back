...
export const availableDisableUser = async (
  ...
  try {
    ...
    if (!id_usuario) {
      ...
      throw new Err.BadRequestError(MESSAGES.ERROR.VALIDATION.MISSING_PARAMETERS);
    }
...
export const blockOrUnblockUser = async (
  ...
  try {
    ...
    if (!userData) {
      logger.warn(`${req.method} ${req.originalUrl} - Usuario no encontrado: ${userId}`);
      return next(new Err.NotFoundError(MESSAGES.ERROR.USER.NOT_FOUND));
    }
...
export const changeUserRole = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (userData.maestros.length > 1) {
      ...
      throw new Err.BadRequestError(MESSAGES.ERROR.USER.MULTIPLE_MAESTRO_RECORDS);
    }
    ...
    if (!rol) {
      logger.warn(`${req.method} ${req.originalUrl} - Rol inválido: ${newRole}.`);
    ...
    if (!authenticatedUserData) {
      logger.warn(`${req.method} ${req.originalUrl} - Usuario no encontrado: ${userAuthId}`);
      return next(new Err.NotFoundError(MESSAGES.ERROR.USER.NOT_FOUND));
    }
...
export const getUsers = async (
  ...
  try {
    ...
    if (id) {
      ...
      if (!user) {
        logger.warn(`${req.method} ${req.originalUrl} - Usuario no encontrado con ID: ${id}`);
        return next(new Err.NotFoundError(MESSAGES.ERROR.USER.NOT_FOUND));
      }

      logger.info(`${req.method} ${req.originalUrl} - Usuario encontrado con éxito con ID: ${id}`);
    ...
    if (filters.nombresApellidos) findFilters.nombresApellidos = String(filters.nombresApellidos);
...
export const getRegistrosPendientes = async (
  ...
  try {
    ...
    if (!pendingUsers) {
      logger.warn(`${req.method} ${req.originalUrl} - Usuario no encontrado: ${userFilters}`);
      return next(new Err.NotFoundError(MESSAGES.ERROR.USER.NOT_FOUND));
    }

    // Si no hay usuarios pendientes, devolver un array vacío
    if (!pendingUsers || (Array.isArray(pendingUsers) && pendingUsers.length === 0)) {
...
export const createUser = async (
  ...
  try {
    ...
    if (!email || !nombres_y_apellidos || !telefono || !rol) {
      logger.warn(`${req.method} ${req.originalUrl} - Faltan parámetros obligatorios.`);
      return next(new Err.BadRequestError(MESSAGES.ERROR.VALIDATION.MISSING_PARAMETERS));
    }
    ...
    if (!authenticatedProductora) {
      logger.warn(`${req.method} ${req.originalUrl} - Usuario sin productora: ${authenticatedData.maestros[0].maestroId}`);
      return next(new Err.NotFoundError(MESSAGES.ERROR.USER.NOT_FOUND));
    }

    // Verificar que el rol del usuario autenticado sea válido
    if (authenticatedRole !== 'productor_principal' && authenticatedRole !== 'admin_principal') {
      logger.warn(
    ...
    if (
      (authenticatedRole === 'productor_principal' && rol !== 'productor_secundario') ||
    ...
    if (!existingUser) {
      logger.warn(`${req.method} ${req.originalUrl} - Usuario no encontrado: ${email}`);
      return next(new Err.NotFoundError(MESSAGES.ERROR.USER.NOT_FOUND));
    }
    ...
    await UsuarioMaestro.create({
      ...
      productora_id: authenticatedRole === 'productor_principal' ? authenticatedProductora.id_productora : null,
...
export const approveApplication = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (
      !authenticatedRole ||
      (authenticatedRole !== 'admin_principal' && authenticatedRole !== 'admin_secundario')
    ) {
      logger.warn(
    ...
    if (!userData) {
      logger.warn(`${req.method} ${req.originalUrl} - UsuarioMaestro no encontrado: ${usuario}`);
      return next(new Err.NotFoundError(MESSAGES.ERROR.USER.NOT_FOUND));
    }    
    ...
    if (!userData.maestros[0].productora) {
      logger.error(`${req.method} ${req.originalUrl} - Datos de productora no encontrada.`);
      return res.status(404).json({
    ...
    if (userData.maestros.length > 1) {
      logger.error(`${req.method} ${req.originalUrl} - El usuario tiene más de una productora asignada.`);
      return res.status(404).json({
...
export const rejectApplication = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
...
export const sendApplication = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const tipoPersonaValida = ['FISICA', 'JURIDICA'].includes(productoraData.tipo_persona);
    if (!tipoPersonaValida) {
      logger.warn(`${req.method} ${req.originalUrl} - Tipo de persona inválido.`);
      throw new Err.BadRequestError('El tipo de persona debe ser FISICA o JURIDICA.');
    }
    ...
    logger.info(`${req.method} ${req.originalUrl} - Productora creada con éxito: ${nuevaProductora.id_productora}`);
...
export const updateApplication = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (!productoraData && (!documentos || documentos.length === 0)) {
      logger.warn(`${req.method} ${req.originalUrl} - No se proporcionaron datos para actualizar.`);
      return next(new Err.BadRequestError(MESSAGES.ERROR.VALIDATION.NO_DATA_PROVIDED));
    }
    ...
    if (productoraData) {
      if (productora) {
        ...
        cambios.push(`Productora creada con ID: ${nuevaProductora.id_productora}`);
      }
    }
    ...
    if (documentos && documentos.length > 0) {
      for (const documento of documentos) {
        ...
        if (documentoExistente) {
          ...
          cambios.push(`Documento actualizado: tipo_documento_id = ${documento.tipo_documento_id}`);
        } else {
          ...
          cambios.push(`Documento creado: tipo_documento_id = ${documento.tipo_documento_id}`);
        }
      }
    }
    ...
    logger.info(`${req.method} ${req.originalUrl} - Auditoría registrada para la aplicación.`);
...
export const deleteUser = async (
  ...
  try {
    ...
    logger.info(`${req.method} ${req.originalUrl} - Usuario eliminado exitosamente: ${id_usuario}`);
